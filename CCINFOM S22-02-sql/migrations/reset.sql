...
DROP TABLE IF EXISTS inventory_txn_log;
...
CREATE TABLE customers (
  ...
  CONSTRAINT ck_customers_phone CHECK (phone IS NULL OR phone REGEXP '^09[0-9]{9}$'),
...
CREATE TABLE employees (
  ...
  updated_at        TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  updated_by        VARCHAR(64) NOT NULL DEFAULT 'system',
  CONSTRAINT ck_employees_phone CHECK (phone IS NULL OR phone REGEXP '^09[0-9]{9}$')
);
...
CREATE TABLE branches (
  ...
  updated_at        TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  updated_by        VARCHAR(64) NOT NULL DEFAULT 'system',
  CONSTRAINT ck_branch_phone CHECK (phone IS NULL OR phone REGEXP '^09[0-9]{9}$'),
...
-- Inventory transaction log (T2/T5)
CREATE TABLE IF NOT EXISTS inventory_txn_log (
...
-- R2 view: weekly picker productivity (non-placeholder)
...
-- R4 view: on-time delivery & PoD compliance
