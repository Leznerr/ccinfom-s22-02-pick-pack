...
set "SQL_DIR=%ROOT%\CCINFOM S22-02-sql"
...
type "%SQL_DIR%\migrations\reset.sql" | "%MYSQL_EXE%" -h %DB_HOST% -P %DB_PORT% -u %DB_USER% -p%DB_PASSWORD%
if errorlevel 1 goto :eof

echo [*] Seeding cores into "%DB_SCHEMA%" ...
type "%SQL_DIR%\seed\cores.sql" | "%MYSQL_EXE%" -h %DB_HOST% -P %DB_PORT% -u %DB_USER% -p%DB_PASSWORD% %DB_SCHEMA%
if errorlevel 1 goto :eof
