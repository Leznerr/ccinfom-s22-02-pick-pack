...
public class ReportR2Form extends JFrame {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            if (rows.isEmpty()) {
                ...
                statusPanel.setSuccess(String.format(
                    ...
                    rows.size(),
                    summary,
                    rows.stream().mapToLong(R2WeeklyProductivityRow::getLinesPicked).sum(),
                    rows.stream().mapToDouble(row -> row.getUnitsPicked().doubleValue()).sum()
                ));
            }
        });
    }
    ...
    private void loadLookupCombos() {
        ...
        try {
            // Load pickers (employees with picker role)
            pickerCombo.removeAllItems();
            pickerCombo.addItem(new ComboItem<>(null, "All Pickers"));
            lookupDao.listActiveEmployees().stream()
                .filter(emp -> "picker".equalsIgnoreCase(emp.getRole()))
                .forEach(picker -> 
