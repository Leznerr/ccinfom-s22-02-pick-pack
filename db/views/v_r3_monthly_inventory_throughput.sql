...
-- Ensure the view exposes the metrics committed in /docs/reports-spec.md 
-- (boxes packed, manifests, inventory deltas, SLA stats).
...
SELECT 
    ...
    DATE_FORMAT(MIN(ch.pod_ts), '%Y-%m') AS `year_month`,
...
LEFT JOIN pack_box_hdr pb 
    ...
    AND YEAR(pb.sealed_at) = YEAR(ch.pod_ts)
    AND MONTH(pb.sealed_at) = MONTH(ch.pod_ts)

LEFT JOIN dispatch_hdr dh 
    ON pth.pick_ticket_id = dh.pick_ticket_id
    AND YEAR(dh.created_at) = YEAR(ch.pod_ts)
    AND MONTH(dh.created_at) = MONTH(ch.pod_ts)

LEFT JOIN inventory_txn_log itl 
    ON itl.ticket_id = pth.pick_ticket_id
    AND itl.source_txn_type = 'CLOSE'
    AND YEAR(itl.created_at) = YEAR(ch.pod_ts)
    AND MONTH(itl.created_at) = MONTH(ch.pod_ts)
...
LEFT JOIN pack_box_hdr pb ON pth.pick_ticket_id = pb.pick_ticket_id 
    AND pb.sealed_flag = TRUE
    AND YEAR(pb.sealed_at) = YEAR(ch.pod_ts)
    AND MONTH(pb.sealed_at) = MONTH(ch.pod_ts)
LEFT JOIN dispatch_hdr dh ON pth.pick_ticket_id = dh.pick_ticket_id
    AND YEAR(dh.created_at) = YEAR(ch.pod_ts)
    AND MONTH(dh.created_at) = MONTH(ch.pod_ts)
LEFT JOIN inventory_txn_log itl ON itl.ticket_id = pth.pick_ticket_id
    AND itl.source_txn_type = 'CLOSE'
    AND YEAR(itl.created_at) = YEAR(ch.pod_ts)
    AND MONTH(itl.created_at) = MONTH(ch.pod_ts)
...
GROUP BY YEAR(ch.pod_ts), MONTH(ch.pod_ts)
...
SELECT 
    ...
    estimated_return_cost
FROM v_monthly_summary
WHERE DATE(CONCAT(`year_month`, '-01')) >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH)
ORDER BY `year` DESC, `month` DESC;
