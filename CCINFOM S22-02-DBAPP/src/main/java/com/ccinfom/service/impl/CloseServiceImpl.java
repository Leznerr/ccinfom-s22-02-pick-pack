...
public class CloseServiceImpl implements CloseService {
    ...
    public void closeTicket(CloseHeader header, List<CloseVariance> variances)
        ...
        try {
            ...
            Map<Long, BigDecimal> pickedQtyByLine = loadPickedQuantitiesByLine(header.getPickTicketId(), conn);
            ...
            for (CloseVariance variance : variances) {
                ...
                BigDecimal pickedQty = pickedQtyByLine.getOrDefault(variance.getTicketLineId(), BigDecimal.ZERO);
                if (variance.getDeliveredQty().compareTo(pickedQty) > 0) {
                    throw new ValidationException(
                            "CLOSE_DELIVER_GT_PICK",
                            "Delivered qty exceeds picked qty for ticket_line_id " + variance.getTicketLineId());
                }

                BigDecimal deltaReserved = pickedQty.negate();
    ...
    private void validateCloseRequest(CloseHeader header, List<CloseVariance> variances)
        ...
        for (CloseVariance variance : variances) {
            ...
            if (variance.getShortQty().compareTo(BigDecimal.ZERO) > 0) {
                hasShort = true;
            }
        }

        if (header.getFinalStatus() == CloseHeader.FinalStatus.Delivered && hasShort) {
            throw new ValidationException("CLOSE_STATUS_MISMATCH",
                    "Cannot mark ticket as Delivered when shortages exist.");
        }

        if (header.getFinalStatus() == CloseHeader.FinalStatus.ShortClosed && !hasShort) {
            throw new ValidationException("CLOSE_STATUS_MISSING_SHORT",
                    "Short-Closed requires at least one line with short quantity.");
        }
    }
    ...
    private Map<Long, BigDecimal> loadPickedQuantitiesByLine(long pickTicketId, Connection conn) throws SQLException {
        final String sql = """
            SELECT pl.ticket_line_id, SUM(pl.picked_qty) AS picked_qty
              FROM picking_line pl
              JOIN picking_hdr ph ON ph.picking_id = pl.picking_id
             WHERE ph.pick_ticket_id = ?
             GROUP BY pl.ticket_line_id
            """;

        Map<Long, BigDecimal> picked = new HashMap<>();
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setLong(1, pickTicketId);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    long lineId = rs.getLong("ticket_line_id");
                    BigDecimal qty = rs.getBigDecimal("picked_qty");
                    if (qty == null) {
                        qty = BigDecimal.ZERO;
                    }
                    picked.put(lineId, qty);
                }
            }
        }
        return picked;
